...
app.listen(4000, function () {
  console.log("Node server is running on 4000 port :) ");
});

app.get("/check", (req, res) => {
  return res.status(200).json({
    message: "Checking wether server is running",
  });
});
...
app.post("/createRecord", async function (req, res, next) {
  try {
    ...
    const { id, patientId, createdBy, title, details, userId } = req.body;
    // const result = await helper.getBalance(walletId);
    const result = await invoke.invokeTransaction(
      "CreateAsset",
      { id, patientId, createdBy, title, details: JSON.stringify(details) },
      userId
    );
...
app.post("/setFaucetWallet", async function (req, res, next) {
  try {
    ...
    const result = await invoke.invokeTransaction(
      "setFaucetWallet",
      { amount: amount, timeDelay: timeDelay, timeStamp: timeStamp },
      userId
    );
    console.log("Response from chaincode", result);
    //check response returned by login function and set API response accordingly
